...
int numPat; // # de l'image dans IVpat
imgu *imgPat; // juste une image de pattern!
...
ImageViews* IV=NULL;
ImageViews* IVpat=NULL; // pour les patterns
...
double ComputeDiff(imgu *A,imgu *B)
{
int x,y,c,n;
int d,err;
	n=0;
...
int NextLowisGood=0; // 0=on attend une grosse difference, 1=garde la prochaine basse

#define LARGE_DIF	4000

imgu *depth=NULL;

//
//
void processMatch(imgu *A,imgu *B,imgu *D)
{
int x,y,c;
int va,vb;
int amb;
...
*/
	...
	//imguCopy(&Iref,I[i]);
		...
		double dif=ComputeDiff(I[i],last);
		printf("dif=%12.6f\n",dif);

		if( dif<-LARGE_DIF ) {
			processMatch(last,I[i],&depth);
			IV->updateViewImage(cap->userNumber,depth);
			imguCopy(&last,I[i]);
		}else if( dif>LARGE_DIF ) {
			processMatch(I[i],last,&depth);
			IV->updateViewImage(cap->userNumber,depth);
			imguCopy(&last,I[i]);
		}
	}
...
int x,y,c;
	if( p==0 ) {
		...
		for(x=0;x<I->xs;x++)
		for(c=0;c<I->cs;c++) {
			PIXEL(I,x,y,c)=(x*65535.0)/(I->xs-1.0)+0.5;
		}
	}else if( p==1 ) {
		for(y=0;y<I->ys;y++)
		for(x=0;x<I->xs;x++)
		...
		for(x=0;x<I->xs;x++)
		for(c=0;c<I->cs;c++) {
			PIXEL(I,x,y,c)=65535-(int)((x*65535.0)/(I->xs-1.0)+0.5);
		}
	}
}
...
char camDevice[100];
	...
	IVpat=new ImageViews(NB_IVpat,1); // avec HUD!
	...
	// image de pattern
	imguAllocate(&imgPat,832,624,3);

	setPattern(imgPat,0);
	numPat=IVpat->addView("pattern",0.0,0.0,1.0,1.0);

	IVpat->updateViewImage(numPat,imgPat);

    // construct the viewer.
    osgViewer::Viewer viewer;

    // the viewer for the patterns
    osgViewer::Viewer viewerPat;

    IVpat->setViewer(&viewerPat);
	...
	IVpat->adjustViewer(1280,0,832,624,false);
...
if( camnum>=0 || camName[0] ) {
   ...
   osg::Group* rootPat = new osg::Group();
   rootPat->addChild(IVpat->getScene());
   viewerPat.setSceneData(rootPat);
...
*/
   ...
   if (!viewerPat.isRealized()) { viewerPat.realize(); }
   ...
   //IV->resetAllViews();
	...
	pthread_create(&threads, &pthread_custom_attr, (void* (*)(void*))background, &viewer);
        ...
        pthread_t threadsPat;
	pthread_attr_init(&pthread_custom_attr);
	pthread_create(&threadsPat, &pthread_custom_attr, (void* (*)(void*))background, &viewerPat);

#ifdef SKIP
	for(i=0;!viewer.done() && !viewerPat.done();i++) {
		//printf("Zzzzz... %d done=%d\n",i,viewer.done());
		sleep(1);
	}
#endif

	double t=getLocalTime();
	int pat=0;
	for(i=0;!viewer.done() && !viewerPat.done();i++) {
		usleep(500000);
		setPattern(imgPat,pat);
		IVpat->updateViewImage(numPat,imgPat);
		if( pat==0 ) pat=1;
		else if( pat==1 ) pat=2;
		else pat=0;
